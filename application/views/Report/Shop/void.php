...
if($mode == 0){
?>
<h1>รายงานการ VOID บิล</h1>
<form method="post">
<input type="hidden" name="mode" id="mode" value="<?=$mode;?>" />
<input type="hidden" name="report_name" value="shop_void_page_<?=$page;?>" />
...
?>
<div style="margin: 0; padding: 0;"><h1>รายงานการ VOID บิล</h1></div>
<br />
...
<?php
	$this->db->select("A.bill_id, A.shop_id, B.bill_no, SUM(C.amount) cash, SUM(D.amount) card, A.void_datetime, A.void_reason, E.emp_id, F.fullname emp_fullname, F.nickname emp_nickname, E.manual_owner_name");
	$this->db->from("shop A");
	...
	$this->db->join("user E", "A.void_by = E.user_id", "LEFT OUTER");
	$this->db->join("employee F", "E.emp_id = F.emp_id", "LEFT OUTER");
	$this->db->where("A.is_void", 1);
...
<tr style="background-color: <?=$backgroundColor;?>;">
	...
	<td style="vertical-align: middle;"><?=$bill_data['bill_no'];?></td>
...
<?php		
			echo $bill_data['void_reason'] . " - โดย ";
			if($bill_data['emp_id'] == "-1") echo $bill_data['manual_owner_name'];
			else echo $bill_data['emp_fullname'] . " (". $bill_data['emp_nickname'] . ")";
?>
	</td>
</tr>
<tr>
	<td colspan=4 style="vertical-align: middle; text-align: left; background-color: <?=$backgroundColor;?>;">
<?php
...
?>
<tr class="tsum">
	<td colspan=2 style="vertical-align: middle; text-align: right;">รวม : </td>
