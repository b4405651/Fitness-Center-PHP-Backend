...
if($mode == 0){
?>
<h1>รายชื่อ และ รายได้ ของ เทรนเนอร์</h1>
<form method="post">
<input type="hidden" name="mode" id="mode" value="<?=$mode;?>" />
<input type="hidden" name="report_name" value="pt_list" />
<table border=0 cellpadding=5 cellspacing=5 style="margin: 5px 0 5px 0;">
<tr class="thead">
	<td style="color: white;">เทรนเนอร์สังกัดสาขา</td>
</tr>
<tr>
...
<tr>
	<td>
		ข้อมูล ณ วันที่ : <input name="on_date" class="datepicker" value="<?=$on_date;?>" />
	</td>
</tr>
</table>
...
?>
<div style="margin: 0; padding: 0;"><h1>รายชื่อ และ รายได้ ของ เทรนเนอร์</h1></div>
<br />
<div style="text-align: left"><b>เทรนเนอร์สังกัดสาขา : </b><?=$branch_str;?></div>
<br />
...
<tr class="thead">
	<td style="vertical-align: middle; text-align: center;" rowspan=3>เทรนเนอร์</td>
	<td style="vertical-align: middle; text-align: center;" rowspan=3>สังกัดสาขา</td>
	<td style="vertical-align: middle; text-align: center;" colspan=2><?="วันที่ " . $date_param[0] . " " . get_month_name($date_param[1], true) . " $date_param[2]";?></td>
	<td style="vertical-align: middle; text-align: center;" colspan=4><?='สะสมทั้งเดือน ' . get_month_name($date_param[1], true) . ' ' . $date_param[2];?></td>
	<td style="vertical-align: middle; text-align: center;" colspan=4><?='สะสมทั้งปี ' . $date_param[2];?></td>
</tr>
<tr class="thead">
	<td style="vertical-align: middle; text-align: center;">Member</td>
	<td style="vertical-align: middle; text-align: center;">PT</td>
	<td style="vertical-align: middle; text-align: center;" colspan=2>Member</td>
	<td style="vertical-align: middle; text-align: center;" colspan=2>PT</td>
	<td style="vertical-align: middle; text-align: center;" colspan=2>Member</td>
	<td style="vertical-align: middle; text-align: center;" colspan=2>PT</td>
</tr>
<tr class="thead">
	<td style="vertical-align: middle; text-align: center; padding: 5px;">ยอดขาย</td>
	<td style="vertical-align: middle; text-align: center; padding: 5px;">ยอดขาย</td>
	<td style="vertical-align: middle; text-align: center; padding: 5px;">ยอดขาย</td>
	<td style="vertical-align: middle; text-align: center; padding: 5px;">ค่าคอม</td>
	<td style="vertical-align: middle; text-align: center; padding: 5px;">ยอดขาย</td>
	<td style="vertical-align: middle; text-align: center; padding: 5px;">ค่าคอม</td>
	<td style="vertical-align: middle; text-align: center; padding: 5px;">ยอดขาย</td>
	<td style="vertical-align: middle; text-align: center; padding: 5px;">ค่าคอม</td>
...
?>
<tr>
	<td colspan=12 style="vertical-align: middle; text-align: center;">ไม่มีข้อมูล</td>
</tr>
<?php
	} else {
		$count = 0;
...
<?php
			...
			$this->db->where("A.pt_emp_id", $emp_data["emp_id"]);
...
<?php
			...
			$data = $this->db->get();
			//if($emp_data["emp_id"] == '3') echo $this->db->last_query() . "<BR>";
			$data = $data->row(0);
			echo number_format($data->amount);
?>
	</td>
...
<?php
			//ยอดขาย PT ของเดือน
			$this->db->select("IFNULL(SUM(B.amount), 0) amount, COMM_PT_AMOUNT(IFNULL(SUM(B.amount), 0), " . $emp_data['emp_id'] . ", '01/$date_param[1]/$date_param[2]', '" . cal_days_in_month(CAL_GREGORIAN,$date_param[1],($date_param[2] - 543)) . "/$date_param[1]/$date_param[2]') comm_amount");
			$this->db->from("member_pt A");
			$this->db->join("member_pt_payment B", "A.member_pt_id = B.member_pt_id", "INNER");
			$this->db->where("A.pt_emp_id", $emp_data["emp_id"]);
			$this->db->where("STR_TO_DATE(receive_datetime,'%Y-%m-%d') BETWEEN STR_TO_DATE('01/$date_param[1]/$date_param[2]','%d/%m/%Y') AND STR_TO_DATE('" . cal_days_in_month(CAL_GREGORIAN,$date_param[1],($date_param[2] - 543)) . "/$date_param[1]/$date_param[2]','%d/%m/%Y')");
			//if($emp_data["emp_id"] == '3') echo $this->db->last_query() . "<BR>";
			$data = $this->db->get();
			$data = $data->row(0);
			echo number_format($data->amount);
...
<?php
			//ค่าคอม PT ของ เดือน
			echo number_format(round($data->comm_amount, 2));
?>
	</td>
	<td>
<?php
			//ยอดขาย Member สะสมทั้งปี
			$this->db->select("IFNULL(SUM(B.amount), 0) amount");
			$this->db->from("member_ext A");
			$this->db->join("member_ext_payment B", "A.member_ext_id = B.member_ext_id", "INNER");
			$this->db->where("A.seller_emp_id", $emp_data["emp_id"]);
...
?>
	...
	<td>
<?php
			//ค่าคอม Member สะสมทั้งปี
			$sum_comm = 0;
			for($i=1; $i<=12; $i++)
			{
				$this->db->select("COMM_MEMBER_AMOUNT(IFNULL(SUM(B.amount), 0)) comm_amount");
				$this->db->from("member_ext A");
				$this->db->join("member_ext_payment B", "A.member_ext_id = B.member_ext_id", "INNER");
				$this->db->where("A.seller_emp_id", $emp_data["emp_id"]);
...
?>
	...
	<td>
<?php
			//ยอดขาย PT สะสมทั้งปี
			$this->db->select("IFNULL(SUM(B.amount), 0) amount");
			$this->db->from("member_pt A");
			$this->db->join("member_pt_payment B", "A.member_pt_id = B.member_pt_id", "INNER");
			$this->db->where("A.pt_emp_id", $emp_data["emp_id"]);
			$this->db->where("STR_TO_DATE(receive_datetime,'%Y-%m-%d') BETWEEN STR_TO_DATE('01/01/$date_param[2]','%d/%m/%Y') AND STR_TO_DATE('31/12/$date_param[2]','%d/%m/%Y')");
			$data = $this->db->get();
			$data = $data->row(0);
			echo number_format($data->amount);
?>
	...
	<td>
<?php
			//ค่าคอม PT สะสมทั้งปี
			$sum_comm = 0;
			for($i=1; $i<=12; $i++)
			{
				$this->db->select("COMM_PT_AMOUNT(IFNULL(SUM(B.amount), 0), " . $emp_data['emp_id'] . ", '01/$i/$date_param[2]', '" . cal_days_in_month(CAL_GREGORIAN,$i,($date_param[2] - 543)) . "/$date_param[1]/$date_param[2]') comm_amount");
				$this->db->from("member_pt A");
				$this->db->join("member_pt_payment B", "A.member_pt_id = B.member_pt_id", "INNER");
				$this->db->where("A.pt_emp_id", $emp_data["emp_id"]);
				$this->db->where("STR_TO_DATE(receive_datetime,'%Y-%m-%d') BETWEEN STR_TO_DATE('01/$i/$date_param[2]','%d/%m/%Y') AND STR_TO_DATE('" . cal_days_in_month(CAL_GREGORIAN,$date_param[1],($date_param[2] - 543)) . "/$i/$date_param[2]','%d/%m/%Y')");
				$data = $this->db->get();
				$data = $data->row(0);
				$sum_comm += $data->comm_amount;
			}
			echo number_format($sum_comm);
?>
	</td>
